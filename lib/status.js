...
import { reconcileDuplicateTasks } from './jsonStore.js';
...
export async function runStatus() {
  ...
  // The signature an interrupted moveTask reassignment leaves behind (see
  // lib/jsonStore.js) — a task present in more than one agent's inbox.
  // Reported, never auto-fixed; picking the authoritative copy is a
  // human call.
  const duplicates = await reconcileDuplicateTasks(config);
  if (duplicates.length > 0) {
    console.log(
      boxen(
        [
          chalk.red.bold(`⚠️  ${duplicates.length} task(s) found in more than one file`),
          '',
          ...duplicates.map(d => `  ${chalk.white(d.taskId)}${d.title ? ` "${d.title}"` : ''}\n    ${chalk.dim(d.files.join(', '))}`),
          '',
          chalk.dim('Likely an interrupted reassignment. Not auto-fixed — pick the'),
          chalk.dim('authoritative copy and remove the other(s) by hand.'),
        ].join('\n'),
