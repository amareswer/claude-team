...
File writes across this project are lock-protected against two things happening at the literal same instant (see README → Concurrency & Locking) — but that doesn't cover the wider case where *you* read a file early in a cycle, do several turns of work, and only write back later. In that gap, another agent, the office UI, or the human can change the same file. Guard every write you make to a shared file (\`MASTER.md\`, \`master.json\`, \`queue.json\`, or any agent's inbox) like this:

1. **Right after reading a file you intend to write back to later**, take a cheap fingerprint of it — run \`shasum -a 256 <file>\` (or \`wc -l <file>\` for a quicker but weaker check) via Bash, or for JSON files just note its \`lastUpdated\` field if it has one. Keep this fingerprint in your working context for the rest of the cycle.
2. **Immediately before you write**, re-check the same fingerprint against the current file. If it no longer matches what you recorded in step 1, someone else modified the file in between — that's a conflict.
3. **On a conflict, do not silently overwrite it.** Instead:
   - **Stop.** Don't perform the write you were about to make.
   - **Log the conflict** to \`.claude-team/docs/changelog.md\`: \`[timestamp] [orchestrator] CONFLICT: <file> changed since last read — write aborted, re-reading before retrying\`.
   - **Notify every agent affected** by appending a message to their inbox's \`messages\` array with \`"type": "conflict"\`:
     \`\`\`json
     { "from": "orchestrator", "type": "conflict", "file": ".claude-team/tasks/master.json", "message": "Detected a change to master.json between my read and write — re-reading before reapplying your task update.", "sentAt": "ISO-timestamp" }
     \`\`\`
     For a file scoped to one agent (their inbox), notify just them. For a shared file (\`MASTER.md\`, \`master.json\`, \`queue.json\`), notify every agent whose task state was part of the write you aborted.
   - **Re-read the file fresh** and re-apply your intended change on top of its *current* contents — merge your change in, don't clobber what changed. Retry the write once. If it conflicts again on the retry, stop retrying and raise it as a question in \`HUMAN_INPUT.md\` instead.

This is a session-level safeguard on top of the file locking every write already goes through — the lock stops two writes colliding at the same moment; this protocol stops you from overwriting a change that landed in the gap between when you read a file and when you got around to writing it.
