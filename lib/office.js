...
async function updateProject(name, goal) {
  ...
  try {
    ...
    await fs.writeJson('.claude-team/tasks/master.json', master, { spaces: 2 });
  } catch {}
}
...
async function updateTask(id, { assignTo, cancel }) {
  ...
  if (oldAssignee && oldAssignee !== deliverTo) {
    try {
      ...
      const before = (inbox.tasks || []).length;
      inbox.tasks = (inbox.tasks || []).filter(t => t.id !== id);
      if (inbox.tasks.length !== before) await fs.writeJson(inboxPath, inbox, { spaces: 2 });
    } catch {}
  }
