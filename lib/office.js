...
import { updateJson, writeJsonLocked, moveTask } from './jsonStore.js';
...
// A blocking single-choice prompt — the one-time "trust this folder?" dialog,
// or the "set up auto mode for your environment?" onboarding nudge Claude
// Code 2.1.232 added — renders as pure silence to the "no output AND no
// input" quiet check above: nothing streams, nothing's been typed. But
// typing into one races with its keystroke handling, and a nudge ending in
// \r can confirm the modal's *default* choice instead of ever reaching the
// model as a real turn — confirmed live: the auto-mode modal defaults to
// "1. Set it up", a real action, not a no-op. Detect generically off the
// shared "Enter to confirm" footer rather than chasing each modal's exact
// wording, since more can appear in future CLI versions. Word-boundary
// matching doesn't work here — the TUI positions words with cursor-column
// jumps instead of printing spaces, so strip everything but letters first.
function looksLikeBlockingPrompt(buffer) {
  const tail = buffer.slice(-4000)
    .replace(/\x1b\[[0-9;?]*[a-zA-Z]/g, '')
    .replace(/\x1b\][^\x07]*\x07/g, '')
    .toLowerCase()
    .replace(/[^a-z]/g, '');
  return tail.includes('entertoconfirm');
}
  ...
  supervisorHandle = setInterval(() => {
    ...
    for (const [name, session] of sessions) {
      if (!session.exited && session.autoLoop) {
        ...
        if (quietFor >= QUIET_MS && sinceNudge >= session.pollIntervalMs && !looksLikeBlockingPrompt(session.buffer)) {
          try {
...
async function flushPersistedTokens() {
  ...
  try {
    ...
    await writeJsonLocked(TOKEN_STORE, { agents: persistedTokens.agents, updatedAt: new Date().toISOString() });
...
export async function runOffice(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      } else if (req.method === 'POST' && req.url === '/api/project') {
        const { name, goal } = await readBody(req);
        await updateProject(String(name || '').trim(), String(goal || '').trim());
        send(200, { ok: true });
      } else if (req.method === 'POST' && req.url === '/api/taskupdate') {
        const { id, assignTo, cancel } = await readBody(req);
        await updateTask(String(id || '').trim(), { assignTo, cancel: !!cancel });
        send(200, { ok: true });
      } else {
  ...
  process.on('SIGINT', () => {
    ...
    if (persistedTokensDirty) {
      // Synchronous shutdown path (must finish before process.exit below),
      // so it can't use the async lock in lib/lock.js. Best-effort and
      // unlocked — acceptable here because token-usage.json is
      // office-only (no agent ever writes it) and this only runs once,
      // at process exit.
      try {
...
async function launchAgent(pty, name, requestedModel) {
  let config = await fs.readJson('.claude-team/config.json');
  const isOrchestrator = name === 'orchestrator';
  let agentEntry = isOrchestrator ? null : config.agents.find(a => a.name === name);
  ...
  if (requestedModel) {
    ...
    config = await updateJson('.claude-team/config.json', (doc) => {
      if (isOrchestrator) doc.orchestratorModel = model;
      else {
        const entry = doc.agents.find(a => a.name === name);
        if (entry) entry.model = model;
      }
    });
    agentEntry = isOrchestrator ? null : config.agents.find(a => a.name === name);
  } else {
...
async function answerQuestion(id, answer) {
  ...
  try {
    ...
    await updateJson(inboxPath, (inbox) => {
...
/**
 * Renames the project / updates its goal — config.json plus the mirror in
 * master.json. Generated docs (MASTER.md etc.) mention the old name in
 * prose; those are the team's living documents, so the orchestrator picks
 * the change up from config rather than us rewriting them.
 */
async function updateProject(name, goal) {
  if (!name) throw Object.assign(new Error('Project name is required'), { statusCode: 400 });
  await updateJson('.claude-team/config.json', (config) => {
    config.projectName = name;
    config.projectGoal = goal;
  });
  try {
    await updateJson('.claude-team/tasks/master.json', (master) => {
      master.projectName = name;
      master.projectGoal = goal;
...
/**
 * Reassigns or cancels a task from the office task board. Mirrors the state
 * everywhere a task lives: master.json, queue.json, and agent inboxes.
 * Reassigning between two real agents is a genuine two-file move (one
 * inbox to another) and goes through `moveTask` — see lib/jsonStore.js —
 * so an interrupted move duplicates the task rather than losing it.
 */
async function updateTask(id, { assignTo, cancel }) {
  if (!id) throw Object.assign(new Error('Task id is required'), { statusCode: 400 });

  const config = await fs.readJson('.claude-team/config.json');
  const agentNames = new Set([...config.agents.map(a => a.name), 'orchestrator']);
  const deliverTo = !cancel && agentNames.has(assignTo) ? assignTo : null;
  const now = new Date().toISOString();
  ...
  for (const file of ['.claude-team/tasks/master.json', '.claude-team/tasks/queue.json']) {
    await withLock(file, async () => {
      let doc;
      try { doc = await fs.readJson(file); } catch { return; }
      const t = (doc.tasks || []).find(t => t.id === id);
      if (!t) return;
      if (['done', 'cancelled'].includes(t.status)) {
        throw Object.assign(new Error(`Task is already ${t.status}`), { statusCode: 400 });
      }
      oldAssignee = oldAssignee || (agentNames.has(t.assignedTo) ? t.assignedTo : null);
      if (cancel) {
        t.status = 'cancelled';
      } else {
        t.assignedTo = deliverTo || 'unassigned';
        t.status = deliverTo ? 'assigned' : 'queued';
      }
      found = t;
      if ('lastUpdated' in doc) doc.lastUpdated = now;
  ...
  if (!found) throw Object.assign(new Error(`No task "${id}" found`), { statusCode: 404 });

  if (oldAssignee && deliverTo && oldAssignee !== deliverTo) {
    // Reassignment between two real agents — a genuine two-file move.
    // moveTask keeps this duplicate-safe (never lossy) instead of the
    // previous remove-then-insert, which could drop the task entirely if
    // the process died between the two writes.
    try {
      await moveTask({
        fromFile: `.claude-team/tasks/${oldAssignee}-inbox.json`,
        toFile: `.claude-team/tasks/${deliverTo}-inbox.json`,
        taskId: id,
        transform: (task) => {
          task.assignedTo = deliverTo;
          task.status = 'assigned';
          task.assignedAt = now;
        },
      });
    } catch (err) {
      // oldAssignee's inbox doesn't actually have it (already removed,
      // never delivered, etc.) — fall back to a plain delivery so the
      // reassignment still lands.
      if (err.code !== 'TASK_NOT_FOUND') throw err;
      await updateJson(`.claude-team/tasks/${deliverTo}-inbox.json`, (inbox) => {
        inbox.tasks = inbox.tasks || [];
        inbox.tasks.push({ ...found, assignedAt: now });
      }, { tasks: [], messages: [] });
    }
    return;
  }

  // Only one side changes — not a cross-file move, no duplicate/loss risk.
  if (oldAssignee && oldAssignee !== deliverTo) {
    try {
      const inboxPath = `.claude-team/tasks/${oldAssignee}-inbox.json`;
      await updateJson(inboxPath, (inbox) => {
        inbox.tasks = (inbox.tasks || []).filter(t => t.id !== id);
      });
    } catch {}
  }

  if (deliverTo && deliverTo !== oldAssignee) {
    const inboxPath = `.claude-team/tasks/${deliverTo}-inbox.json`;
    await updateJson(inboxPath, (inbox) => {
      inbox.tasks = inbox.tasks || [];
      inbox.tasks.push({ ...found, assignedAt: now });
...
async function collectState(canLaunch) {
  ...
  // Task summary + full board from master.json
  ...
  try {
    ...
    const all = master.tasks || [];
    taskSummary = {
      total: all.length,
      done: all.filter(t => t.status === 'done').length,
      inProgress: all.filter(t => ['in_progress', 'assigned'].includes(t.status)).length,
      queued: all.filter(t => ['queued', 'not_started'].includes(t.status)).length,
      paused: all.filter(t => t.status === 'paused').length,
    };
    tasks = all.map(t => ({
      id: t.id, title: t.title, description: t.description || '',
      priority: t.priority || 'medium', assignedTo: t.assignedTo || 'unassigned',
      status: t.status || 'queued', createdAt: t.createdAt || null,
    }));
  } catch {}
...
async function readAgent(agent) {
  ...
  // Full outbox history for the drawer (the shared feed only shows the
  // 8 most recent across the whole team).
  let completedWork = [];
  try {
    const outbox = await fs.readJson(`.claude-team/tasks/${agent.name}-outbox.json`);
    completedWork = (outbox.completedTasks || [])
      .map(t => ({ title: t.title || '', summary: t.summary || '', completedAt: t.completedAt || '' }))
  ...
  return {
    ...
    // Same detector the watchdog guard uses (see looksLikeBlockingPrompt) —
    // true while the agent's terminal shows a modal (trust-folder dialog,
    // "set up auto mode?") waiting on a human, distinct from the agent's own
    // self-reported status since it can't update that file mid-modal.
    needsHumanPrompt: session && !session.exited ? looksLikeBlockingPrompt(session.buffer) : false,
    tokenUsage,
  };
}
