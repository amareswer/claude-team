...
export async function runInit() {
  ...
  console.log(chalk.cyan.bold('  Step 1 of 5 — Project Basics\n'));
  ...
  console.log();
  console.log(chalk.cyan.bold('  Step 2 of 5 — Team Hierarchy\n'));
  ...
  console.log();
  console.log(chalk.cyan.bold('  Step 3 of 5 — Worker Agents\n'));
  ...
  const allAgents = [
    ...leadershipRoles.map(r => ({ name: r.name, role: r.role, responsibilities: r.responsibilities, isLeadership: true })),
    ...workerAgents,
  ];

  // ── Step 4: Coordination ──────────────────────────────────────────────────
  console.log();
  console.log(chalk.cyan.bold('  Step 4 of 5 — Coordination\n'));
  ...
  // ── Step 5: First task ────────────────────────────────────────────────────
  console.log();
  console.log(chalk.cyan.bold('  Step 5 of 5 — First Task\n'));
  ...
  if (addTask) {
    const assignChoices = [
      { name: '🎯  Project Manager (will plan and delegate)', value: 'project-manager' },
  ...
  console.log(boxen(
    [
      ...
      ...leadershipRoles.map(r => chalk.dim(`  ${r.icon}  ${r.name.padEnd(18)}`) + chalk.cyan(`.claude-team/agents/${r.name}.md`)),
      '',
      chalk.dim('Workers:'),
      ...workerAgents.map(a => chalk.dim(`  🤖  ${a.name.padEnd(18)}`) + chalk.cyan(`.claude-team/agents/${a.name}.md`)),
      '',
      chalk.dim('Docs:'),
