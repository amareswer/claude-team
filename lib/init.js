...
import { generateReviewChecklistMd } from './generators/docs/reviewChecklist.js';
...
export async function runInit() {
  ...
  const basics = await inquirer.prompt([
    ...
    {
      type: 'select', name: 'projectCategory',
  ...
  const { hierarchyType } = await inquirer.prompt([{
    type: 'select', name: 'hierarchyType',
  ...
  if (teamSize === 'custom') {
    ...
    workerCount = customCount;
  }

  // Cost/scale check — hierarchy + worker count are both settled now, before
  // asking for per-worker details, so declining doesn't waste the human's
  // time answering questions for a team size they're about to reconsider.
  const totalAgents = 1 + leadershipRoles.length + workerCount; // +1 = orchestrator, always present
  console.log();
  console.log(
    chalk.dim(`  Team: 1 orchestrator + ${leadershipRoles.length} leadership + ${workerCount} worker${workerCount === 1 ? '' : 's'} = `)
    + chalk.white.bold(`${totalAgents} agent${totalAgents === 1 ? '' : 's'}`) + chalk.dim(' total.')
  );
  console.log(chalk.yellow(
    `  ⚠️  That's ${totalAgents} full Claude Code session${totalAgents === 1 ? '' : 's'} running concurrently once everyone's launched — `
    + `each is its own subscription session with its own context window and token usage. Launch only the agents a task actually needs.`
  ));
  console.log();
  const { confirmTeamSize } = await inquirer.prompt([{
    type: 'confirm', name: 'confirmTeamSize',
    message: `Continue with ${totalAgents} agents?`,
    default: true,
  }]);
  if (!confirmTeamSize) {
    console.log(chalk.dim('\n  Init cancelled. Re-run `claude-team init` and choose a smaller team.\n'));
    return;
  }
  ...
  const { modelStrategy } = await inquirer.prompt([{
    type: 'select', name: 'modelStrategy',
  ...
  if (modelStrategy === 'recommended') {
    ...
    const { model } = await inquirer.prompt([{
      type: 'select', name: 'model', message: 'Model for every agent:',
    ...
    for (const r of modelRoster) {
      const { model } = await inquirer.prompt([{
        type: 'select', name: 'model', message: `  Model for ${r.name} (${r.role}):`,
  ...
  const coordination = await inquirer.prompt([
    ...
    {
      type: 'confirm', name: 'enableCrossSessionMessaging',
      message: 'Enable experimental cross-session messaging (requires Claude Code 2.1.224+)? Lets agents message each other\'s live sessions directly instead of only via inbox files — but the underlying tool sees every Claude Code session on this machine, not just this team, so an agent could receive a message from (or send one to) an unrelated session.',
      default: false,
    },
  ]);
  ...
  if (addTask) {
    ...
    const t = await inquirer.prompt([
      ...
      { type: 'select', name: 'priority', message: 'Priority:', choices: ['high', 'medium', 'low'], default: 'high' },
      { type: 'select', name: 'assignTo', message: 'Assign to:', choices: assignChoices },
    ]);
    initialTask = t;
  }
  ...
  const config = {
    ...
    coordination: {
      ...
      enableCrossSessionMessaging: !!coordination.enableCrossSessionMessaging,
    },
  ...
  try {
    ...
    spinner.text = 'Generating REVIEW_CHECKLIST.md...';
    await fs.writeFile('.claude-team/docs/REVIEW_CHECKLIST.md', generateReviewChecklistMd(config));
  ...
  console.log(boxen(
    [
      ...
      chalk.dim('  ✅  REVIEW_CHECKLIST.md') + chalk.cyan(' .claude-team/docs/REVIEW_CHECKLIST.md'),
      '',
