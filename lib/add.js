...
// Agent names become filesystem path segments in half a dozen places below
// (and in office.js) — `.trim().toLowerCase().replace(/\s+/g, "-")` alone
// only collapses whitespace, it doesn't strip `/`, `\`, or `..`. A name like
// "../../../../tmp/pwned" survives that untouched and, interpolated into
// `.claude-team/agents/${name}.md`, resolves outside the project entirely.
// Validated here (the shared choke point for both `claude-team add`/`remove`
// and the office's /api/hire //api/remove, which pass request bodies
// straight through) so nothing downstream needs to re-check it.
const VALID_AGENT_NAME = /^[a-z0-9-]+$/;
function assertSafeAgentName(name) {
  if (!VALID_AGENT_NAME.test(name)) {
    throw Object.assign(
      new Error(`Invalid agent name "${name}" — use only lowercase letters, numbers, and hyphens`),
      { statusCode: 400 }
    );
  }
}
...
export async function addAgentToTeam(config, { name, role, responsibilities, model }) {
  ...
  await writeJsonLocked(`.claude-team/tasks/${agent.name}-inbox.json`, { tasks: [], messages: [] });
  await writeJsonLocked(`.claude-team/tasks/${agent.name}-outbox.json`, { completedTasks: [] });
